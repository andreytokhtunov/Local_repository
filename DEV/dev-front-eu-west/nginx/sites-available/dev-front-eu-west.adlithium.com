...
upstream backend_event_server {
    server 10.132.0.3:26001 max_fails=0 fail_timeout=10s;
    keepalive 512;

}
upstream backend_cs_server {
    server 10.132.0.3:15000 max_fails=0 fail_timeout=10s;
    keepalive 512;
}

# Supply sides backend servers.
upstream backend_bid_openx {
    server 10.132.0.3:12339 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_zedo {
    server 10.132.0.3:12350 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_adaptv {
    server 10.132.0.3:12351 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_smartyads {
    server 10.132.0.3:12352 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_google {
    server 10.132.0.3:12353 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_vertamedia {
    server 10.132.0.3:12354 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_epom {
    server 10.132.0.3:12355 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_smaato {
    server 10.132.0.3:12356 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_adforge {
    server 10.132.0.3:12357 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_mmg {
    server 10.132.0.3:12358 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_vrtcal {
    server 10.132.0.3:12359 max_fails=0 fail_timeout=10s;
    keepalive 512;
}
upstream backend_bid_wazimo {
    server 10.132.0.3:12360 max_fails=0 fail_timeout=10s;
    keepalive 512;
}

server {
    listen 10.132.0.3:80;
    server_name     dev-front-eu-west.adlithium.com;
    root            /home/rtb/adm;
    index index.php;
